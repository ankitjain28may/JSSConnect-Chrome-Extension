...
$(document).ready(function() {
    ...
    function college() {
        ...
        $(".notices").append($("<div class='col-sm-12 text-center preloader-main'></div>").append($("<img class='preloader' src='assets/connect.gif'>")));
        var delay = 1500; //1.5 second
        setTimeout(function() {
            $.get(htp, function(data, status) {
                if (status == "success" && flag == 1) {
                    ...
                    var link = $("<a href=" + htp + " class='link' target='_blank'></a>").text("Read more");
                    col.append(link);
                    $(".notices").append(col);
                }
            });
        ...
        setTimeout(function() {
            $.get(htp, function(data, status) {
                if (status == "success") {
                    ...
                    var col = $("<div class='text-center' ></div>").addClass("col-sm-12 link-col");
                    var link = $("<a href=" + htp + " class='link' target='_blank'></a>").text("Read more Notices");
                    col.append(link);
                    $(".notices").append(col);
                }
            });
